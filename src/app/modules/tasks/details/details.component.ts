...
import { FtmFindByKeyPipe } from '@ftm/pipes/find-by-key';
...
import { Tag, Task } from 'app/modules/tasks/tasks.types';
import { assign } from 'lodash-es';
import { DateTime } from 'luxon';
import { Subject, debounceTime, filter, take, takeUntil, tap, switchMap } from 'rxjs';
...
export class TasksDetailsComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    constructor(
        ...
        private _confirmationService: FtmConfirmationService,
    ...
    ngOnInit(): void {
        ...
        // Ensure tasks are loaded, then load selected task by id from route
        const id = this._activatedRoute.snapshot.paramMap.get('id');
        this._tasksService.tasks$
            .pipe(takeUntil(this._unsubscribeAll), take(1))
            .subscribe((tasks) => {
                const load$ = tasks ? this._tasksService.getTaskById(id) : this._tasksService.getTasks().pipe(switchMap(() => this._tasksService.getTaskById(id)));
                load$.pipe(takeUntil(this._unsubscribeAll)).subscribe();
            });
        ...
        this._tasksService.task$
            ...
            .subscribe((task: Task) => {
                ...
                if (task) {
                    ...
                    // When task cannot be resolved (e.g., wrong id), navigate back
        ...
        this._router.events
            ...
            .subscribe(() => {
                // Focus on the title field if available
