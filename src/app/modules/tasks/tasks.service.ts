            ...
            switchMap((tasks) => {
                ...
                if (!tasks) {
                    return this.getTasks().pipe(
                        switchMap((loaded) => this.getTaskById(id))
                    );
                }

                const task = tasks.find((item) => item.id === id) || null;

                // Update selected task (may be null)
                this._task.next(task);

                return task ? of(task) : throwError('Could not found task with id of ' + id + '!');
            })
        );
    }
...
// -------------------------------------------------------------------------------------------------
// Route resolvers (functional) - kept in service file for simplicity and reuse
// -------------------------------------------------------------------------------------------------

export const resolveTasks = () => inject(TasksService).getTasks();

export const resolveTags = () => inject(TasksService).getTags();
...
export const resolveTaskById = (
    ...
    return tasksService.getTaskById(id).pipe(
        catchError((error) => {
            // Navigate back to the parent URL if task cannot be resolved
