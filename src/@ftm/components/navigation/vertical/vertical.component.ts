...
import {
    FtmNavigationItem,
    FtmVerticalNavigationAppearance,
    FtmVerticalNavigationMode,
    FtmVerticalNavigationPosition,
} from '@ftm/components/navigation/navigation.types';
import { FtmVerticalNavigationAsideItemComponent } from '@ftm/components/navigation/vertical/components/aside/aside.component';
...
import { FtmVerticalNavigationGroupItemComponent } from '@ftm/components/navigation/vertical/components/group/group.component';
...
import { FtmScrollbarDirective } from '@ftm/directives/scrollbar/scrollbar.directive';
...
@Component({
    selector: 'ftm-vertical-navigation',
    templateUrl: './vertical.component.html',
    styleUrls: ['./vertical.component.scss'],
    animations: ftmAnimations,
    encapsulation: ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush,
    exportAs: 'ftmVerticalNavigation',
    imports: [
        FtmScrollbarDirective,
        FtmVerticalNavigationAsideItemComponent,
        ...
        FtmVerticalNavigationGroupItemComponent,
        FtmVerticalNavigationSpacerItemComponent,
    ],
})
export class FtmVerticalNavigationComponent
    ...
    @Input() appearance: FtmVerticalNavigationAppearance = 'default';
    @Input() autoCollapse: boolean = true;
    @Input() inner: boolean = false;
    @Input() mode: FtmVerticalNavigationMode = 'side';
    @Input() name: string = this._ftmUtilsService.randomId();
    @Input() navigation: FtmNavigationItem[];
    @Input() opened: boolean = true;
    @Input() position: FtmVerticalNavigationPosition = 'left';
    @Input() transparentOverlay: boolean = false;
    @Output()
    readonly appearanceChanged: EventEmitter<FtmVerticalNavigationAppearance> =
        new EventEmitter<FtmVerticalNavigationAppearance>();
    @Output() readonly modeChanged: EventEmitter<FtmVerticalNavigationMode> =
        new EventEmitter<FtmVerticalNavigationMode>();
    @Output() readonly openedChanged: EventEmitter<boolean> =
        new EventEmitter<boolean>();
    @Output()
    readonly positionChanged: EventEmitter<FtmVerticalNavigationPosition> =
        new EventEmitter<FtmVerticalNavigationPosition>();
    @ViewChild('navigationContent') private _navigationContentEl: ElementRef;

    activeAsideItemId: string | null = null;
    onCollapsableItemCollapsed: ReplaySubject<FtmNavigationItem> =
        new ReplaySubject<FtmNavigationItem>(1);
    onCollapsableItemExpanded: ReplaySubject<FtmNavigationItem> =
        new ReplaySubject<FtmNavigationItem>(1);
    ...
    private _ftmScrollbarDirectives!: QueryList<FtmScrollbarDirective>;
    private _ftmScrollbarDirectivesSubscription: Subscription;
    ...
    @HostBinding('class') get classList(): any {
        /* eslint-disable @typescript-eslint/naming-convention */
        return {
            'ftm-vertical-navigation-animations-enabled':
                this._animationsEnabled,
            [`ftm-vertical-navigation-appearance-${this.appearance}`]: true,
            'ftm-vertical-navigation-hover': this._hovered,
            'ftm-vertical-navigation-inner': this.inner,
            'ftm-vertical-navigation-mode-over': this.mode === 'over',
            'ftm-vertical-navigation-mode-side': this.mode === 'side',
            'ftm-vertical-navigation-opened': this.opened,
            'ftm-vertical-navigation-position-left': this.position === 'left',
            'ftm-vertical-navigation-position-right':
                this.position === 'right',
        };
        /* eslint-enable @typescript-eslint/naming-convention */
    }
    ...
    /**
     * Setter for ftmScrollbarDirectives
     */
    @ViewChildren(FtmScrollbarDirective)
    set ftmScrollbarDirectives(
        ftmScrollbarDirectives: QueryList<FtmScrollbarDirective>
    ) {
        // Store the directives
        this._ftmScrollbarDirectives = ftmScrollbarDirectives;

        // Return if there are no directives
        if (ftmScrollbarDirectives.length === 0) {
            return;
        }

        // Unsubscribe the previous subscriptions
        if (this._ftmScrollbarDirectivesSubscription) {
            this._ftmScrollbarDirectivesSubscription.unsubscribe();
        }

        // Update the scrollbars on collapsable items' collapse/expand
        this._ftmScrollbarDirectivesSubscription = merge(
            ...
            .subscribe(() => {
                ...
                ftmScrollbarDirectives.forEach((ftmScrollbarDirective) => {
                    ftmScrollbarDirective.update();
                });
            });
    }
    ...
    ngAfterViewInit(): void {
        ...
        setTimeout(() => {
            ...
            if (
                ...
                const activeItem =
                    this._navigationContentEl.nativeElement.querySelector(
                        '.ftm-vertical-navigation-item-active'
                    );
            ...
            // Otherwise
                ...
                this._ftmScrollbarDirectives.forEach(
                    (ftmScrollbarDirective) => {
                        // Skip if not enabled
                        if (!ftmScrollbarDirective.isEnabled()) {
                            return;
                        }

                        // Scroll to the active element
                        ftmScrollbarDirective.scrollToElement(
                            '.ftm-vertical-navigation-item-active',
                            -120,
                            true
                        );
     ...
     * @param item
     */
    openAside(item: FtmNavigationItem): void {
     ...
     * @param item
     */
    toggleAside(item: FtmNavigationItem): void {
        // Toggle
    ...
    private _showOverlay(): void {
        ...
        this._overlay.classList.add('ftm-vertical-navigation-overlay');

        // Add a class depending on the transparentOverlay option
        if (this.transparentOverlay) {
            this._overlay.classList.add(
                'ftm-vertical-navigation-overlay-transparent'
            );
        }
    ...
    private _showAsideOverlay(): void {
        ...
        this._asideOverlay.classList.add(
            'ftm-vertical-navigation-aside-overlay'
        );
