...
import { FTM_MOCK_API_DEFAULT_DELAY } from '@ftm/lib/mock-api/mock-api.constants';
import { FtmMockApiService } from '@ftm/lib/mock-api/mock-api.service';
...
export const mockApiInterceptor = (
    ...
    const defaultDelay = inject(FTM_MOCK_API_DEFAULT_DELAY);
    const ftmMockApiService = inject(FtmMockApiService);

    // Try to get the request handler
    const { handler, urlParams } = ftmMockApiService.findHandler(
