...
import {
    ScrollbarGeometry,
    ScrollbarPosition,
} from '@ftm/directives/scrollbar/scrollbar.types';
 ...
 */
@Directive({
    selector: '[ftmScrollbar]',
    exportAs: 'ftmScrollbar',
    standalone: true,
})
export class FtmScrollbarDirective implements OnChanges, OnInit, OnDestroy {
    /* eslint-disable @typescript-eslint/naming-convention */
    static ngAcceptInputType_ftmScrollbar: BooleanInput;
    /* eslint-enable @typescript-eslint/naming-convention */

    private _elementRef = inject(ElementRef);
    private _platform = inject(Platform);

    @Input() ftmScrollbar: boolean = true;
    @Input() ftmScrollbarOptions: PerfectScrollbar.Options;
    ...
    ngOnChanges(changes: SimpleChanges): void {
        // Enabled
        if ('ftmScrollbar' in changes) {
            // Interpret empty string as 'true'
            this.ftmScrollbar = coerceBooleanProperty(
                changes.ftmScrollbar.currentValue
            );

            // If enabled, init the directive
            if (this.ftmScrollbar) {
                this._init();
            }
        ...
        // Scrollbar options
        if ('ftmScrollbarOptions' in changes) {
            // Merge the options
            this._options = merge(
                {},
                this._options,
                changes.ftmScrollbarOptions.currentValue
            );
